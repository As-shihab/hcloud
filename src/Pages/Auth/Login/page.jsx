...
export default function page() {
  ...
  const hcloud = async ()=>{
    setLoader(true)
   await http.post('login-user' , user)
   .then(res=>{
  ...
  return (
   ...
   <span className='text-2xl'>Welcome dude</span><br />
   <span className='my-2 '>Don't have account ? <button className='text-sky-300 border-b border-sky-600'><Link to="/auth/signup">SignUp</Link></button></span>
   </div>
       </div>
...
<div className="signup text-center lg:w-9/12 w-[90%]  absolute backdrop-blur-lg z-10 top-2/4 left-2/4 -translate-y-2/4 border border-slate-300 rounded-lg shadow-sm p-5 -translate-x-2/4">
  ...
  <span className='text-xl font-semibold text-gray-600'>Welcome back!</span>
<br />
...
<div className="mt-3 w-full flex justify-between items-center">
  <span>{msg&&msg}</span>
  <button onClick={hcloud} className='top-1 right-4 py-2 px-3 rounded-lg shadow-sm bg-blue-400 text-white'>{loader? "Wait..." :"Login"}</button>
</div>
</div>
       </div>
    </div>
